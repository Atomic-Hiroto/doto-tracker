...
export async function turboFacts(message: Message) {
  try {
    const rows = crewEntries(false).filter(e => e.estimate.rankedMMR != null && e.estimate.lean != null);
    const all = crewEntries(false);
    if (rows.length < 3) return message.reply('Need at least 3 crew players with visible ranked medals for Turbo facts.');

    const highQuality = rows.filter(r => !r.estimate.partyFallback && r.estimate.confidence >= 70 && r.estimate.soloSampleSize >= 15);
    const avgLean = rows.reduce((s, r) => s + (r.estimate.lean ?? 0), 0) / rows.length;
    const over = rows.filter(r => (r.estimate.lean ?? 0) >= 250).length;
    const under = rows.filter(r => (r.estimate.lean ?? 0) <= -250).length;
    const top = all[0];
    const topName = top ? await nameFor(message, top) : 'n/a';
    const bestLean = [...rows].sort((a, b) => (b.estimate.lean ?? 0) - (a.estimate.lean ?? 0))[0];
    const worstLean = [...rows].sort((a, b) => (a.estimate.lean ?? 0) - (b.estimate.lean ?? 0))[0];

    const embed = new EmbedBuilder()
      .setColor('#f97316')
      .setTitle('🧪 Turbo Facts')
      .setDescription('Quick takeaways from the current crew Turbo calibration dataset.')
      .addFields(
        {
          name: 'Dataset',
          value:
            `Crew estimates: **${all.length}** | with visible ranked medals: **${rows.length}**\n` +
            `High-quality solo estimates: **${highQuality.length}/${rows.length}**\n` +
            `Average Turbo lean: **${fmtLean(avgLean)}**`,
          inline: false,
        },
        {
          name: 'Current Story',
          value:
            `Top hidden Turbo estimate: **${topName}** — ${top?.estimate.medal ?? 'n/a'}\n` +
            `Overperformers (+250 or more): **${over}** | underperformers (-250 or worse): **${under}**\n` +
            `Biggest Turbo lean: **${bestLean.steamName ?? bestLean.steamId}** (${fmtLean(bestLean.estimate.lean)})\n` +
            `Biggest ranked-over-Turbo gap: **${worstLean.steamName ?? worstLean.steamId}** (${fmtLean(worstLean.estimate.lean)})`,
          inline: false,
        },
        {
          name: 'Interpretation',
          value: 'Ranked skill transfers into Turbo, but Turbo compresses skill gaps and has a top-end ceiling. Lean is the fun stat: it shows who plays above or below their visible medal in Turbo.',
          inline: false,
        },
      )
      .setTimestamp();
    ...
    logger.error('Error in turbofacts:', error);
    await message.reply('Failed to build Turbo facts.');
  }
}
